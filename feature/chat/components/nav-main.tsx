...
import NavRoomItem from "./nav-room-item";
...
export function NavMain() {
  ...
  // const [dataSource, setDataSource] = useState<ChatNavGroup[]>([]);
  ...
  useEffect(() => {
    const fetchChatGroups = async () => {
      if (session?.user?.userId) {
        const chatCategorys = await getUserRooms();
        console.log(chatCategorys);
        setChatCategorys(chatCategorys, session.user.userId);
      }
    };
    fetchChatGroups();
  }, [session?.user?.userId]);
          ...
          {chatCategorys.map((record) => {
            return (
              <div key={record.id} className="flex " data-id={record.id}>
                {isEditMode && record.id !== "dm" && record.id !== "group" && (
                  <Button
                    variant="ghost"
